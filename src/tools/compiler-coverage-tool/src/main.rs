...
struct FunctionReport {
    ...
    line_start: usize,
    _line_end: usize,
    // None = LLVM doesn't track this line; Some(n) = hit count (summed across monos)
...
fn main() -> Result<()> {
    ...
    for func in &functions {
        ...
        // kind != 0 means expansion/skipped/gap — skip those, only kind=0 is real code.
        //
        // Per line: pick the innermost (tightest) region's count. LLVM nests branch
        // regions inside block regions; taking the minimum instead causes outer counts
        // to bleed into lines only reachable via specific branches.
        // Tightness = smallest line span; ties broken by latest rs.
        ...
        // None = LLVM doesn't track this line; also treat bug!/span_bug! lines as None
        // since those are intentionally unreachable.
        ...
        // LLVM maps branch-not-taken counters onto closing braces; promote them
        // if the preceding covered line in this function was covered.
        ...
        // TODO: propagate uncovered to match arm patterns (grey but body is red).
        // naive lookahead over-marks, needs a smarter approach.
...
fn merge_monomorphizations(reports: Vec<FunctionReport>) -> Vec<FunctionReport> {
    // same (filename, line_start) = same generic function instantiated multiple times;
    // sum counts so coverage from any mono contributes.
